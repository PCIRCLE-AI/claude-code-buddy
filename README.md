...
- An active supported host may receive a native message push, removing polling for that live delivery. It never wakes a stopped session: persistence, host acceptance, acknowledgement, and workflow state remain independent. See the [owner-private Local runner setup](docs/platforms/agent-messaging.md#one-time-owner-private-local-runner-setup) for the one-time Codex, Claude, and ACP runner configuration, or [agent messaging](docs/platforms/agent-messaging.md) for principal/session/generation routing and the Local versus Cloud boundary.
