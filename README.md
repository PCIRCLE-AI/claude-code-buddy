...
- An active supported managed host may receive a native message push, removing polling for that live delivery. It never wakes a stopped session: persistence, host acceptance, acknowledgement, and workflow state remain independent. See the [owner-private managed-host setup](docs/platforms/agent-messaging.md#one-time-owner-private-managed-host-setup) for reusable Codex, Claude, and Gemini configuration, or [agent messaging](docs/platforms/agent-messaging.md) for principal/session/generation routing and the Local versus Cloud boundary.
...
- Durable recovery is still local and pull-based: a stopped or unsupported receiver must keep or restart a poll/watch loop. Native delivery does not resume a stopped model session, execute its payload, or imply acknowledgement.
