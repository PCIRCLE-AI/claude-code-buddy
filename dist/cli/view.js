...
function queryData(dbPath) {
    ...
    try {
        const tables = db
            .prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('entities', 'observations', 'relations', 'tags')")
            .all()
            .map(r => r.name);
        ...
        const tagRows = tables.includes('tags')
            ? db.prepare('SELECT entity_id, tag FROM tags').all()
            : [];
        const relationRows = tables.includes('relations')
            ? db
                .prepare('SELECT from_entity_id, to_entity_id, relation_type FROM relations')
                .all()
            : [];
        ...
        const relations = relationRows
            .filter((r) => entityMap.has(r.from_entity_id) && entityMap.has(r.to_entity_id))
            .map((r) => ({
...
export function generateDashboardHtml(dbPath) {
