...
export const TOOL_DEFINITIONS = [
    ...
    {
        name: 'message',
        description: 'Exchange durable local agent messages on one MeMesh instance. send creates one message/delivery/wakeup event idempotently; poll waits or catches up with an opaque cursor; fetch reads the payload; intake, ack, disposition, and activation record separate explicit facts. Polling or fetching never acknowledges a message, and no action executes payload content.',
        inputSchema: {
            type: 'object',
            properties: {
                action: {
