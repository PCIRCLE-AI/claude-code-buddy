...
**🧠 スマート検索** — 「login security」で検索すると「OAuth PKCE」についてのメモリが見つかります。MeMesh はホットパスで FTS5 + sqlite-vec を使い、LLM を使いません。ベクトル補完により関連する表現にも届きます。

**🌏 分かち書きしない文字体系での検索** — 中国語・日本語・韓国語・タイ語・ラオ語・クメール語・半角カタカナは隣り合う 2 文字の組として索引化されます。そのため「資料庫遷移前一定要先備份」と書いた記憶は、全文を正確に打ち込まなくても「備份」で見つかります。書き込み側と検索側の両方で NFC 正規化を行うので、macOS や韓国語・ベトナム語の IME で入力した記憶もどちらの表記でも見つかります。
...
MeMesh はデフォルトでオフライン動作します — リコールは厳密に LLM フリーのまま(箱出し状態で LongMemEval-S 95.60% R@5)。LLM API キーを追加するのは、その上に LLM 拡張の分析フローを重ねたい場合のみです: より賢いセッション抽出、新規メモリの自動タグ付け、失敗からのレッスン生成、`dream` 圧縮:
