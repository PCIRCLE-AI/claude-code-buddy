...
**Search**:
- `search(query?, opts?)` -- FTS5 MATCH query with optional tag filtering; tracks access on returned entities. Query terms are OR-ed (a bare space is FTS5's implicit AND, which required every word of a question to appear in one memory) and rows are ordered by FTS5 `rank` (BM25), not by id — `LIMIT` decides what survives to the scorer, so ordering by id discarded the best match before it could be scored. Terms are capped at `MAX_QUERY_TERMS` (32) so a pasted log dump cannot build an unbounded disjunction. `buildQueryTerms()` splits on the boundaries `unicode61` itself uses — `[\p{L}\p{N}\p{M}]+` over an NFC-normalised query — rather than on whitespace, so `kitchen's` does not become the phrase `kitchen s`, decomposed text stays whole, and CJK queries survive. The optional tag filter is an `EXISTS` subquery rather than a join, so one statement serves every filter combination without `SELECT DISTINCT`. Note `unicode61` treats an unbroken CJK run as one token, so substring matching within it needs a trigram-tokenizer migration.
