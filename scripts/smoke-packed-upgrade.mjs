...
// this one proves that an actual npm-global 4.8.0 install keeps its local
// data when npm replaces it with the exact packed 4.8.1 candidate.
const repoRoot = process.cwd();
const expectedPreviousVersion = '4.8.0';
const expectedCandidateVersion = '4.8.1';
...
const target = args.indexOf('@pcircle/memesh@4.8.1');
...
`, { mode: 0o700 });
  ...
  assert.match(success.stdout, /START target=4\.8\.1/);
  assert.match(success.stdout, /SUCCESS target=4\.8\.1 installed=4\.8\.1/);
  ...
  assert.ok(callLines.includes('install -g @pcircle/memesh@4.8.1'), 'runner did not request the exact candidate version');
      ...
      if (mode === 'seed') {
        const saved = await client.callTool({
          ...
          arguments: {
            ...
            type: 'fact',
            observations: ['This marker was written by the installed v4.8.0 MCP server before upgrade.'],
            tags: ['packed-upgrade'],
          },
        });
        assert.notEqual(saved.isError, true, 'v4.8.0 MCP remember failed');
      } else {
        ...
        assert.match(JSON.stringify(recalled), /packed-upgrade-existing-memory/,
          'candidate MCP could not read the v4.8.0 memory');
      }
    } finally {
      await client.close();
    }
...
try {
  ...
  assert.equal(installed.packageJson.version, expectedPreviousVersion,
    'the isolated baseline is not the public v4.8.0 package');
  const oldCli = installedBin(prefix, 'memesh');
  const oldMcp = installedBin(prefix, 'memesh-mcp');
  assert.ok(fs.existsSync(oldCli), 'v4.8.0 npm-global CLI is missing');
  assert.equal(installed.packageJson.bin?.['memesh-mcp'], 'dist/mcp/server.js',
    'public v4.8.0 package does not declare the expected MCP binary');
  assert.ok(fs.existsSync(oldMcp), 'public v4.8.0 tarball did not install the memesh-mcp binary');
  ...
  assert.equal(installed.packageJson.version, expectedCandidateVersion,
    'npm did not replace the isolated global install with v4.8.1');
  ...
  assert.match(JSON.stringify(recalledByCli), /packed-upgrade-existing-memory/,
    'candidate CLI could not read the v4.8.0 memory');
  ...
  // make the v4.8.0-era data unreadable.
  if (autoUpdate) {
    ...
    assert.match(failed.stderr, /FAILED target=4\.8\.1 stage=install-or-readback/);
    assert.equal(fs.existsSync(autoUpdate.lockPath), false, 'failed auto-update left its lock behind');
  } else {
    npmGlobalInstall(path.join(upgradeRoot, 'not-a-memesh-v4.8.1-candidate.tgz'), prefix, env, true);
  }
  installed = readInstalledPackage(prefix, env);
  assert.equal(installed.packageJson.version, expectedCandidateVersion,
    'failed upgrade changed the existing v4.8.1 package');
  ...
  console.log('failure-path: auto-update install failure was reported without SUCCESS; v4.8.1 CLI and pre-existing data remain readable');
  console.log('✅ Packaged upgrade smoke passed — public v4.8.0 npm-global data survived the packed v4.8.1 auto-update runner and its exact installed-version readback.');
} finally {
