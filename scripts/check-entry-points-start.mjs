...
import { binEntries, hookEntries } from './lib/executable-targets.mjs';
...
async function main() {
   ...
   * raw Node stack trace. Verified empirically (2026-09-02): `claude.js`
   * meets this (main() at claude.ts:278-285 wraps the call in try/catch and
   * prints "memesh-host-claude: session startup failed."); `codex.js` and
   * `acp.js` do NOT — both call `await run…Host()` at top-level with no
   * try/catch (codex.ts:299-302, acp.ts:449-452) and Node dumps the raw
   * exception with stack frames. This assertion is written to the spec's
   * required contract, not to today's behaviour, so it fails honestly for
   * those two — see the run report for the real output. */
  ...
  try {
    ...
    const bins = binEntries(packageDir);
    const hooks = hookEntries(packageDir);
    ...
    const entries = [
      ...bins.map((b) => ({ label: `bin ${b.name} (${b.relativePath})`, relativePath: b.relativePath })),
      ...hooks.map((h) => ({ label: `hook ${h.event}${h.async ? ' async' : ''} (${h.relativePath})`, relativePath: h.relativePath })),
    ];
