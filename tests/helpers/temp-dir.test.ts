/**
 * `removeTempDir` swallows errors. This pins WHICH ones.
 *
...
describe('removeTempDir', () => {
  it('actually removes the directory when nothing is racing it', () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'memesh-temp-dir-'));
  ...
  it('rethrows anything else, so a genuinely broken cleanup is still visible', () => {
    ...
    expect(() => removeTempDir('/nonexistent')).toThrow(/simulated EACCES/);
  });

  it('removes every directory it is given, not just the first', () => {
    const a = fs.mkdtempSync(path.join(os.tmpdir(), 'memesh-temp-dir-a-'));
    const b = fs.mkdtempSync(path.join(os.tmpdir(), 'memesh-temp-dir-b-'));
