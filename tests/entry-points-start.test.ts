...
import { binEntries, hookEntries } from '../scripts/lib/executable-targets.mjs';
...
describe('executable-targets derivation picks up new manifest entries', () => {
  ...
  // consumer share — rather than only at the whole-gate level, so a future
  // fourth consumer inherits the same guarantee.
  it('binEntries reflects a bin entry no fixture author hand-listed here, with its name preserved', () => {
    ...
    expect(entries).toContainEqual({ name: 'memesh-brand-new-thing', relativePath: 'dist/brand-new-thing.js' });
  });

  it('hookEntries reflects a hook command no fixture author hand-listed here, with event + async preserved', () => {
    const root = fixtureRoot();
    ...
    expect(entries).toContainEqual({
      event: 'PreCompact',
      async: true,
      command: '${CLAUDE_PLUGIN_ROOT}/scripts/hooks/brand-new-hook.js',
      relativePath: 'scripts/hooks/brand-new-hook.js',
    });
  });
});
...
describe('gate fails when a declared entry point does not start', () => {
  ...
  it('a broken bin fails the gate while an unrelated working hook still passes', () => {
    ...
    expect(result.stdout).toContain('ok    hook PreToolUse (scripts/hooks/pre-edit-recall.js)');
  });
